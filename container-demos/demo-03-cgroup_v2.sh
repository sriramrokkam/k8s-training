...
p "# let's create a new cgroup that governs CPU usage (but also other things)"
pe "cd /sys/fs/cgroup"
pe "sudo mkdir ${CGROUP_NAME}"
pe "cd ${CGROUP_NAME}"
...
p "# we put two of our three dd processes into this group"

sudo chmod 666 cgroup.procs cpu.max
k=0
for i in `pidof dd`; do
	pe "echo $i | sudo tee cgroup.procs"
	k=$((k+1))
	[ $k -eq 2 ] && break
done

p "# Now we can see our processes in the cgroup"
pe "cat cgroup.procs"
p "# we can now control how many CPU shares these processes get by altering the values in cpu.max"
pe "cat cpu.max"
p "# watch how the following command affects the CPU consumption of our dd's"
pe 'echo "50000 100000" > cpu.max'
pe "cat cpu.max"
p "# this is used to limit resources to individual processes or process groups - very useful for containers"
p "# But we can so much more about cgroups..."
